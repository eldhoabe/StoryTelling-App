...
This is not a kids app. It is closer to a nature documentary you hold in your hand at bedtime — visual, calm, narrated, and rooted entirely in real science. The child does not need to touch the screen. The parent is the storyteller. The phone is the campfire.
...
5. They read aloud to their child — or let the narration voice play. A card appears: a full-screen, photorealistic illustration with one or two sentences at the bottom.
...
Narration is the most important feature differentiator. It is what makes BioGraph feel like a documentary and not a slideshow.

### MVP narration

In the first version, narration uses the device's built-in text-to-speech. The voice reads each card's body text aloud, then pauses for a few seconds — giving the parent and child time to look at the image together — before the next card appears. The parent can also turn narration off and read aloud themselves, in which case a gentle visual cue prompts them to begin reading.

### V2 narration

In the second version, narration is replaced with pre-recorded audio using a professional warm documentary voice. This audio is generated when a story is added to the system and stored so it plays instantly, consistently, and with the right tone on every device. This is when BioGraph starts sounding like *Planet Earth*.
...
- Orchestrating narration — either device TTS in MVP or audio files in V2
...
| 4 | Narration | Device narration plays per card, pauses between cards, advances automatically |
| 5 | Bedtime mode | Bedtime toggle dims screen, slows transitions, enlarges text, hides chrome |
| 6 | Particle atmosphere | Ash particles render in the Cretaceous world without performance issues |
| 7 | World portal transition | Tapping a story node triggers the cinematic entry animation |
| 8 | Knowledge Graph home screen | Graph renders all worlds, pan and zoom works, tapping enters a world |
| 9 | Unlock system | Completing a story unlocks connected worlds with an animation |
| 10 | Full content load | All 10 Phase 1 stories loaded, illustrated, fact-checked, and readable |
| 11 | Launch | Web live on Vercel, API live on Railway, mobile testable via Expo Go |
...
1. **Pre-rendered narration audio** — warm documentary voice via ElevenLabs; the single biggest product improvement
